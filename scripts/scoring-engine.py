...
def run_scoring():
    # (criteria v2.1) مسار البيانات يقبل معاملاً اختيارياً — الافتراضي يُشتق من موقع السكربت
    # فيعمل في الإنتاج (/srv/ideas) والمختبر معاً دون تعديل
    import os as _os
    _root = _os.path.dirname(_os.path.dirname(_os.path.abspath(__file__)))
    data_path = sys.argv[1] if len(sys.argv) > 1 else _os.path.join(_root, 'stocks-data.json')
